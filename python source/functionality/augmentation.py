...
def get_augment_matrix(rot=0.0, shift_x=0.0, shift_y=0.0, zoom_x=0.0, zoom_y=0.0):
    """Builds the augmentation matrix from the given parameters.
       Builds inverted operation matrix as well."""
    ...
    # Combine matrices by matrix multiplication
...
def augment_data(image, labels):
    """Randomly transforms the input data for augmentation purposes."""
    ...
    # - bias vector: serves as supportive vector for shift operation (allows the operation in absence of addition)
    ...
    bias_vec = tf.ones([IMAGE_SIZE[0] * IMAGE_SIZE[1]], dtype='int32')
    coordinates = tf.stack([x, y, bias_vec])

    # Find new pixel coordinates
    # 1. Multiply coordinates and augmentation matrix
    # 2. Saturate coordinates exceeding the space of the image
